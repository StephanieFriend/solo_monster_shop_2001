...
RSpec.describe 'As a Merchant Employee' do
  it 'I can see a list of all my merchants bulk discounts' do
    ...
    expect(current_path).to eq("/merchant/#{@regina.merchant_id}/bulk_discounts/#{@discount1.id}/edit")

    fill_in :percentage, with: 15
    click_button 'Update Discount Rate'

    expect(current_path).to eq("/merchant/#{@regina.merchant_id}/bulk_discounts")

    within "#discount-#{@discount1.id}" do
      expect(page).to have_content("Percentage Off: 15")
      expect(page).to have_no_content("Percentage Off: 10")
